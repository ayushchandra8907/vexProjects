...
void opcontrol() {
	...
	while (true) {
		LB.move((controller.get_analog(ANALOG_RIGHT_Y) + controller.get_analog(ANALOG_RIGHT_X))*1);
		RB.move((controller.get_analog(ANALOG_RIGHT_Y) - controller.get_analog(ANALOG_RIGHT_X))*1);
		LF.move((controller.get_analog(ANALOG_RIGHT_Y) + controller.get_analog(ANALOG_RIGHT_X))*1);
		RF.move((controller.get_analog(ANALOG_RIGHT_Y) - controller.get_analog(ANALOG_RIGHT_X))*1);
		...
		pros::lcd::print(5, "Left Encoder: %f", LB.get_position());
		pros::lcd::print(6, "Right Encoder: %f", RB.get_position());
...
void moveToPoint(float targetX, float targetY) {
	...
	while (!atPoint) {
		...
		LB.move((power + turnPower)*1);
		RB.move((power - turnPower)*1);
